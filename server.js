...
function scanExactOccurrences(taggedText, target, source, type) {
  ...
  // "언급 안 됨"으로 취급돼 부당하게 반려된다 — 짧은 이름에서 글자가
  // 두 개까지 틀어지는 오독도 잡아야 하므로 관대한 쪽으로 기운다(실측:
  // "우르오스"가 "오로스"로 들린 사례의 편집거리가 3이었음). 근접
  // 매치는 자동 통과가 아니라 "확인 필요" 배지만 붙고 마케터가 최종
  // 확인하므로, 넓혀서 생기는 비용은 확인 항목이 조금 느는 정도다 —
  // 그 반대(부당한 반려)가 훨씬 크다. 경쟁 브랜드는 근접 매치를 넓히면
  // 없는 위반을 의심하게 되므로 기존 기준(minDist=1)을 그대로 유지한다.
  const minDist = type === "brand" || type === "product" ? 3 : 1;
...
금칙 항목이 "비듬이 생긴다", "떡진다"처럼 특정 증상이 "생기거나 심해진다"는 형태로 적혀있다면, 이는 "이 제품
때문에 그 증상이 생기거나 악화됐다"는 부정적 후기를 금지하려는 것이다. 방향을 반드시 구분하라 — 같은 단어(예:
"비듬")가 나와도 "비듬 개선 효과", "비듬 없이 깨끗해졌어요"처럼 증상이 좋아졌다는 방향이면 이는 금지하려는
부정적 후기가 아니라 오히려 정상적인 홍보이므로 위반이 아니다. 단어의 등장 여부가 아니라 "악화됐다"인지
"개선됐다"인지 문장의 방향으로 판단하라.
